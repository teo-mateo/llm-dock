...
- **Metrics:** the template passes `--metrics`, but `/api/services/<service>/metrics` gates on `template_type in ("vllm", "llamacpp", "ninfer")` and returns `metrics: {}` for `ik_llamacpp`, while `ServiceDetailsPage.jsx` enables the panel for it — so the tab renders empty rather than erroring
...
- **Image:** `llm-dock-ninfer`, compiled from source by `ninfer/Dockerfile` + `./build-ninfer.sh`. Pinned by `ARG NINFER_COMMIT` plus two patches: `ninfer/ninfer-cu129-port.patch` (upstream wants CUDA 13.1, this host's driver (575.x) tops out at CUDA 12.9, and CUDA ≤ 12.9 rejects the four kernel families whose staged shared memory exceeds the 48 KiB static cap — the patch moves those onto the dynamic-shared path) and `ninfer/ninfer-metrics.patch` (adds the authed `GET /metrics` route below). Both are plain `git diff`s of the pinned tree applied at build time; bump the pin and the port patch together. The metrics patch retires separately, when upstream ships a stats endpoint of its own.
